...
class startAndEndOf{
  ...
  //게임 끝
  void end({required bool doWin}){
    if(doWin) print("승리하셨습니다!");
    else print('3회 이상 입력에 실패하셨습니다. 끝말잇기에서 패배하셨습니다');
...
class WordInputer{
  ...
  var word = File('/Users/hangayun/implude_dart_practice/main/assets/단어목록.txt',).readAsLinesSync();
  late List<String> nextWordList;
  late List<String?> usedWord;
  bool writeAgrin = false;
  late String imputWord;
  late var crt_word;
  var life = ['♥ ', '♥ ', '♥ '];

  ///////////////////////////////////////////////////// 함수 선언 /////////////////////////////////////////////////////
  void getInputWord extends (){
    print("$crt_word\n");
    print("체력: ${life.join()}");
    stdout.write("\" ${crt_word.split('').last} \" (으)로 시작하는 단어를 입력하세요! => ");

    //단어 입력 받기
    imputWord = stdin.readLineSync() ?? '';
  }
  ...
  void isValidWord(){
      ...
      // 모두 한글로만 이루어져 있는가
      bool isAllKorean = crt_word.split('').every((a) => a.codeUnitAt(0) >= 0xAC00 && a.codeUnitAt(0) <= 0xD7A3,);
...
void main() {
  ...
  do {
    ...
    do {
      ...
      var kindOfErrors = (imputWord == '')? [[imputWord == '', "단어가 입력되지 않았습니다."],]: [//null이 입력됐을 때
        [imputWord.split('').every((a) =>a.codeUnitAt(0) < 0xAC00 || a.codeUnitAt(0) > 0xD7A3,),"단어는 한글로만 이루어져 있어야 합니다. ",],
        [imputWord.length < 2, '단어는 두 글자 이상이여야 합니다. '],
        [usedWord.indexOf(imputWord) != -1, "이미 사용된 적이 있는 단어입니다. "],
        [word.indexOf(imputWord) == -1 && imputWord != '',"존재하지 않는 단어입니다. ",],
        [usedWord.last!.split('').last != imputWord.split('').first,"전 단어의 끝말과 입력하신 단어의 첫말이 맞지 않습니다.",],
        [imputWord.split('').last == '다', "명사만을 입력 해주세요"],
      ];
      print(
        '\n—————————————————————————————————————————————————————————————————————————————————————\n',
      );
    ...
    usedWord.add(imputWord);

    nextWordList = word
        .where((word) => word.startsWith(imputWord.split('').last))
        .toList();
